...
CREATE PROCEDURE [dbo].[STPLANTYP_ACIONA_OLOS_WC_REPRO] AS  
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------  
 ...
 SELECT @REPICDIASUTEIS_CONF = CAST(INFORMACAO AS INT) FROM  SRC.DBO.CONFIGURA_VERTICAL (NOLOCK) WHERE CAMPO = 'REPICDIASUTEIS_CONF'  
  
   -- PEGAR O EVENTO CASO O PERCENTUAL DO TELEFONE SEJA ZERO E TENHA A OPÇÃO USAR EVENTO MARCADA  
 SELECT @ACIONARNEGATIVARTELEFONE_CONF = CAST(INFORMACAO AS INT) FROM SRC.DBO.CONFIGURA_VERTICAL (NOLOCK) WHERE CAMPO = 'ACIONARNEGATIVARTELEFONE_CONF'  
 -- PEGAR O EVENTO CASO O PERCENTUAL DO TELEFONE SEJA ZERO E TENHA A OPÇÃO USAR EVENTO MARCADA  
  
  -- CONFIGURAÇÃO FEITA PARA LIMITAR O PERCENTUAL ATÉ 100%  
 SELECT @LIMITAPERCTEL100_CONF = COALESCE(CAST(INFORMACAO AS INT), 0) FROM SRC.DBO.CONFIGURA_VERTICAL (NOLOCK) WHERE CAMPO = 'LIMITAPERCTEL100_CONF'  
...
INSERT  
 ...
 SRC.DBO.ACIONA(CONTRATO_FIN,DATA_ACIONA,COD_ACIONAMENTO,AGENDA_ACIONA,COD_RECUP,TEL_TEL,COD_TIPAC,ARQUIVO_SOM,DATA_ACIONA2,  
...
RECUP_AGENDA, DATA_AGENDA, EXPORTADO_ACIONA, CANCELADO_AGENDA, TELA_ACIONA  
 ...
 SELECT   
   B.ID, A.IDCALL, 'OLOS'  
 FROM  
  CONFIGURACAO_OLOS_WC_REPRO  AS A JOIN #ACIONA_ID AS B ON A.CONTRATO_FIN = B.CONTRATO_FIN AND A.[DATA_ACIONA] = B.[DATA_ACIONA]     
   
  
   UPDATE AUX SET QTD_ACIONAMENTOS_HUMANO = COALESCE(QTD_ACIONAMENTOS_HUMANO, 0) + 1, DATA_ULTIMO_ACIONAMENTO_HUMANO = B.DATA_ACIONA  
     
   FROM   
    SRC.DBO.AUX_DEVF AS AUX (NOLOCK)   
    JOIN CONFIGURACAO_OLOS_WC_REPRO AS B ON AUX.CONTRATO_FIN = B.CONTRATO_FIN  
    JOIN SRC.DBO.CAD_ACIONAMENTO AS C (NOLOCK) ON C.COD_ACIONAMENTO = B.COD_ACIONAMENTO  
   WHERE   
  ...
  BEGIN  
   ...
   FROM   
    CONFIGURACAO_OLOS_WC_REPRO AS A   
  ...
  BEGIN  
   ...
   FROM   
    SRC.DBO.CAD_BLOQUEIO  AS A (NOLOCK)  
    JOIN CONFIGURACAO_OLOS_WC_REPRO AS B ON A.CONTRATO_FIN = B.CONTRATO_FIN  
    JOIN SRC.DBO.CAD_ACIONAMENTO AS C (NOLOCK) ON B.COD_ACIONAMENTO = C.COD_ACIONAMENTO  
   WHERE   
    BLACKLIST_ACIONAMENTO = 0  
...
------------------------------------------------------------------------------------------------------------------------------------------------  
   ...
   FROM   
    SRC.DBO.TBL_CONTROLEEMUSO AS A (NOLOCK)  
    JOIN CONFIGURACAO_OLOS_WC_REPRO AS U ON A.CONTRATO_FIN = U.CONTRATO_FIN  
    JOIN SRC.DBO.CAD_DEVF (NOLOCK) AS E ON A.CONTRATO_FIN = E.CONTRATO_FIN  
    LEFT JOIN SRC.DBO.CAD_STCB AS B (NOLOCK) ON B.COD_STCB = E.COD_STCB--TABELA DOS ATUAIS  
    LEFT JOIN SRC.DBO.CAD_ACIONAMENTO AS C (NOLOCK) ON C.COD_ACIONAMENTO = U.COD_ACIONAMENTO  
    LEFT JOIN SRC.DBO.CAD_STCB AS D (NOLOCK) ON C.COD_STCB = D.COD_STCB -- TABELA DOS NOVOS  
...
--- ALTERA O TEMMPO DE REPIC  
  ...
  SET   
   ...
   FROM   
    SRC.DBO.TBL_CONTROLEEMUSO AS A (NOLOCK)  
    JOIN CONFIGURACAO_OLOS_WC_REPRO AS B ON A.CPF_DEV = B.CPF_DEV   
    JOIN SRC.DBO.CAD_ACIONAMENTO AS C (NOLOCK) ON C.COD_ACIONAMENTO = B.COD_ACIONAMENTO  
    JOIN SRC.DBO.CAD_DEVF AS D (NOLOCK) ON D.CONTRATO_FIN = A.CONTRATO_FIN  
    LEFT JOIN SRC.DBO.CAD_STCB AS E (NOLOCK) ON E.COD_STCB = D.COD_STCB -- COD_STCB ATUAL  
    LEFT JOIN SRC.DBO.CAD_STCB AS F (NOLOCK) ON F.COD_STCB = C.COD_STCB -- COD_STCB NOVO  
   WHERE               
...
---- ALTERA A DATA DO ULTIMO ACIONAMENTO---  
  ...
  FROM  
   ...
   JOIN SRC.DBO.CAD_ACIONAMENTO AS D (NOLOCK) ON D.COD_ACIONAMENTO = B.COD_ACIONAMENTO  
   LEFT JOIN SRC.DBO.CAD_STCB AS E (NOLOCK) ON E.COD_STCB = A.COD_STCB--TABELA DOS ATUAIS  
   LEFT JOIN SRC.DBO.CAD_STCB AS F (NOLOCK) ON F.COD_STCB = D.COD_STCB -- TABELA DOS NOVOS  
   LEFT JOIN SRC.DBO.CAD_PRIORSTCB AS G (NOLOCK) ON G.COD_STCB = F.COD_STCB  
   JOIN SRC.DBO.CAD_RECUP AS H (NOLOCK) ON H.COD_RECUP = A.COD_RECUP   
   JOIN SRC.DBO.TBL_CONTROLEEMUSO AS I (NOLOCK) ON I.CONTRATO_FIN = A.CONTRATO_FIN  
  WHERE                  
   FLAGALTERDTULTAC = 0      
   AND ((COALESCE(E.PESO_STCB,0) <=  F.PESO_STCB)  
   OR (COALESCE(E.LIMITATENT_STCB, 1) = 0  
...
--ALTERA DATA DO ÚLTIMO ACIONAMENTO VALIDAÇÃO DE PESO DIFERENTE  
 ...
 FROM  
  SRC.DBO.CAD_DEVF AS A (NOLOCK)  
  JOIN CONFIGURACAO_OLOS_WC_REPRO AS X ON X.CONTRATO_FIN = A.CONTRATO_FIN  
  ...
  LEFT JOIN SRC.DBO.CAD_STCB AS B (NOLOCK) ON A.COD_STCB =B.COD_STCB--TABELA DOS ATUAIS  
  LEFT JOIN SRC.DBO.CAD_ACIONAMENTO AS C (NOLOCK) ON C.COD_ACIONAMENTO = X.COD_ACIONAMENTO  
  LEFT JOIN SRC.DBO.CAD_STCB AS D (NOLOCK) ON C.COD_STCB = D.COD_STCB -- TABELA DOS NOVOS  
  JOIN SRC.DBO.TBL_CONTROLEEMUSO AS E (NOLOCK) ON E.CONTRATO_FIN = A.CONTRATO_FIN  
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------  
  ...
  FROM SRC.DBO.TBL_CONTROLEEMUSO AS A (NOLOCK)  
  JOIN CONFIGURACAO_OLOS_WC_REPRO AS H ON A.CONTRATO_FIN = H.CONTRATO_FIN  
  JOIN SRC.DBO.CAD_ACIONAMENTO AS I (NOLOCK) ON H.COD_ACIONAMENTO = I.COD_ACIONAMENTO  
  LEFT JOIN SRC.DBO.CAD_STCB AS J (NOLOCK) ON J.COD_STCB = I.COD_STCB  
  JOIN SRC.DBO.CAD_DEVF AS K (NOLOCK) ON K.CONTRATO_FIN = H.CONTRATO_FIN  
  LEFT JOIN SRC.DBO.CAD_STCB AS L (NOLOCK) ON L.COD_STCB = K.COD_STCB  
  WHERE   
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------  
 ...
 FROM   
  SRC.DBO.CAD_DEVT (NOLOCK) AS A   
  JOIN CONFIGURACAO_OLOS_WC_REPRO AS B ON A.CPF_DEV = B.CPF_DEV AND A.COD_TEL = B.COD_TEL  
  JOIN SRC.DBO.CAD_ACIONAMENTO (NOLOCK) AS C ON B.COD_ACIONAMENTO = C.COD_ACIONAMENTO  
 WHERE               
 ...
 FROM   
  ...
  JOIN CONFIGURACAO_OLOS_WC_REPRO AS Y ON Y.CPF_DEV = A.CPF_DEV AND Y.COD_TEL = A.COD_TEL  
  JOIN SRC.DBO.CAD_ACIONAMENTO AS C (NOLOCK) ON C.COD_ACIONAMENTO = Y.COD_ACIONAMENTO  
 WHERE   
