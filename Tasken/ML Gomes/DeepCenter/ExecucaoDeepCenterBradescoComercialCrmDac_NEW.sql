...
,LOJISTA     VARCHAR(50)                                                    
 ...
 /***************************************** DROP TABLE SELEÇÕES *******************************************/        
 IF OBJECT_ID ('SRC.DeepCenter.ACIONA_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.ACIONA_BRADESCO_SELECAO                                                    
 END                                                    
                                                 IF OBJECT_ID ('SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_1') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_1                                                    
 END                                                    
                               
 IF OBJECT_ID ('SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2                                                    
 END                                                    
                                                  
 IF OBJECT_ID ('SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO                                                    
 END                                                    
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO') IS NOT NULL                           
 BEGIN                                              
  DROP TABLE SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO                                                    
 END                                                   
                             
 IF OBJECT_ID ('SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO                            
 END                                                    
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO                                                    
 END                                                    
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                   
  DROP TABLE SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO                                                    
 END                                                    
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO') IS NOT NULL                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO                                                    
 END                                                    
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO') IS NOT NULL                                         
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO                           
 END     
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                            
  DROP TABLE SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO                                                    
 END              
                        
 IF OBJECT_ID ('SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO                                             
 END                                            
                                   
 IF OBJECT_ID ('SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX                                                    
 END                                   
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC') IS NOT NULL           
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC                                                    
 END                                                    
                                                    
 IF OBJECT_ID ('SRC.DeepCenter.TBL_JUNCAO_ACIONA_DISCADOR') IS NOT NULL                   
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TBL_JUNCAO_ACIONA_DISCADOR                                                    
 END                                                    
                                                  
 IF OBJECT_ID ('SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO') IS NOT NULL                                                    
 BEGIN                                                    
  DROP TABLE SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO                                                    
 END                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.ACIONA_BRADESCO_SELECAO') IS NULL                                                    
 BEGIN                                    
  WITH CTE AS (           
  SELECT                                                     
  ...
  WHERE                                                     
 DATA_ACIONA BETWEEN @DATA AND @DATA2 AND                                                  
 ...
 INTO SRC.DeepCenter.ACIONA_BRADESCO_SELECAO                                    
 FROM CTE A                                    
 WHERE SEQNUM = 1                                    
 AND NOT EXISTS (SELECT * FROM SRC.DeepCenter.TBL_CONTROLE_DEEPCENTER_CRMDAC AS C WHERE A.ID_ACIONA = C.ID_ACIONA)                                    
  ...
  CREATE NONCLUSTERED INDEX CL_IX ON SRC.DeepCenter.ACIONA_BRADESCO_SELECAO(ID_ACIONA)                                                    
  CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.ACIONA_BRADESCO_SELECAO(CONTRATO_FIN)                              
  CREATE NONCLUSTERED INDEX NON_IX10 ON SRC.DeepCenter.[ACIONA_BRADESCO_SELECAO] ([TEL_TEL])                                                     
 ...
 IF OBJECT_ID ('SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO') IS NULL                                        
 BEGIN                                              
 ...
 FROM                                                     
  ...
  EXISTS (SELECT * FROM SRC.DeepCenter.ACIONA_BRADESCO_SELECAO AS B WHERE A.ID_ACIONA = B.ID_ACIONA)                                                    
                                                  
 SELECT                                           
  * INTO SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO                                                    
 FROM (                                                  
 ...
 CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO (ID_ACIONA) INCLUDE (IDCALL)                                                    
 CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO(IDCALL)                                                 
 CREATE NONCLUSTERED INDEX NON_IX2 ON SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO(IDCALL, DISCADOR)                                                  
 END                                              
 ...
 IF OBJECT_ID ('SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2') IS NULL                                                    
 BEGIN                                           
SELECT      
   ...
   INTO SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2                                                    
   FROM                     
   SRC.[DEEPCENTER].DISCAGENS_OLOS AS A (NOLOCK)                                                    
  WHERE                                                    
   EXISTS (SELECT * FROM SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO (NOLOCK) AS B WHERE A.IDCALL = B.IDCALL /*AND A.CONTRATO = B.CONTRATO_FIN*/)                                                
  ...
  CREATE NONCLUSTERED INDEX NON_IX  ON SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2 (IDCALL)                                 
  --CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2 (CONTRATO)                                                      
 END                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO') IS NULL                                                    
 BEGIN                               SELECT                                                   
  CALLRESULTID AS IDCALL_SINERGYTECH, TEMPO_FALADO AS TEMPOFALANDO, TEMPO_DISCAGEM AS TEMPOCHAMADA                                                  
  INTO SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO                                                
 FROM TBL_RESULTADODISCAGEM_SINERGYTECH (NOLOCK) AS A                                                  
 WHERE EXISTS (SELECT * FROM SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO (NOLOCK) AS B WHERE A.CALLRESULTID = B.IDCALL AND B.DISCADOR = 'SINERGYTECH')                                                  
 ...
 CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO (IDCALL_SINERGYTECH) INCLUDE(TEMPOFALANDO, TEMPOCHAMADA)                                                  
 END;                                                  
 ...
 IF OBJECT_ID ('SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO') IS NULL     
 BEGIN                                                    
  SELECT                                                     
   ...
   ,B.EMAIL_DEV                                
   ,A.PLANO_FIN                                
   INTO SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO                                          
  FROM                                                     
  ...
  WHERE                                                     
   EXISTS (SELECT * FROM SRC.DeepCenter.ACIONA_BRADESCO_SELECAO AS B WHERE A.CONTRATO_FIN = B.CONTRATO_FIN)                                    
   AND A.COD_CLI IN (3,11,16,17) --) OR (A.COD_CLI = 16 AND B.COD_TIPESS = 1))                                    
                                                   
  SET @QTD_REGISTROS = @@ROWCOUNT                                                    
                                                    
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO (CONTRATO_FIN)                                                    
  INCLUDE(CPF_DEV, COD_CLI, COD_CAR, ATRASO_FIN, DTENTRADA_FIN, VALOR_FIN )                                                    
                                          
  CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO (CPF_DEV)                                                    
  INCLUDE(CONTRATO_FIN, COD_CLI, COD_CAR, ATRASO_FIN, DTENTRADA_FIN, VALOR_FIN )                                                    
                                                    
  CREATE NONCLUSTERED INDEX NON_IX2 ON SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO (CONTRATO_FIN)                                                    
  CREATE NONCLUSTERED INDEX NON_IX3 ON SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO (CPF_DEV)                                                    
 END                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO') IS NULL                                     
 BEGIN                                                    
  SELECT                                                     
   CONTRATO_FIN, CONTRATO_ORIGINAL                                                    
   INTO SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO                                                    
  FROM                                                    
   [SRC].DBO.AUX_DEVF AS A  (NOLOCK)                                                    
  WHERE                                                     
   EXISTS (SELECT * FROM SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO AS B WHERE A.CONTRATO_FIN = B.CONTRATO_FIN)                                        
                                          
  SET @QTD_REGISTROS = @@ROWCOUNT                                                    
                                                    
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO (CONTRATO_FIN) INCLUDE (CONTRATO_ORIGINAL)                                                    
 END                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO') IS NULL                                                    
 BEGIN                                                    
  SELECT                                                     
  CONTRATO_FIN                 
   ...
   ,PARCELA_ACOP                                
   ,TIPO_PARCACO                                                    
   INTO SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO                                                    
  FROM                                                     
  ...
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO (CONTRATO_FIN, PARCELA_ACOP, TIPO_PARCACO)                                                    
  INCLUDE (VALOR_ACOP)                                      
                                       
  CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO (CONTRATO_FIN, VALOR_ACOP)                                         
 END                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO') IS NULL                                                    
 BEGIN                                        
  SELECT                                                     
   CONTRATO_FIN                                                    
   ,SUM(VALOR_ACOP) AS VLRATRASO                    
   ,COUNT(*) AS PARCELAATRASO                                  
   INTO SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO                                                    
  FROM                                                     
   SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO AS A (NOLOCK)                                                    
  ...
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO (CONTRATO_FIN)                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO') IS NULL                                           
 BEGIN                     
 SELECT CPF_DEV, DDD_TEL, TEL_TEL, PERC_TEL, POSSUIWHATSAPP_TEL, COD_TEL                                                  
   INTO SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO                                          
  FROM                                                     
   [SRC].DBO.CAD_DEVT  AS A  (NOLOCK)                                                    
  WHERE                                                     
   EXISTS (SELECT * FROM SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO AS B WHERE A.CPF_DEV = B.CPF_DEV)                                                    
   AND PERC_TEL > 0                                                  
         
  SET @QTD_REGISTROS = @@ROWCOUNT                                                    
                                
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO (CPF_DEV, DDD_TEL, TEL_TEL)                                                    
  CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO (CPF_DEV) INCLUDE (DDD_TEL, TEL_TEL, PERC_TEL, POSSUIWHATSAPP_TEL, COD_TEL)                                                    
 ...
 BEGIN                                                    
  ...
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO (CPF_DEV) INCLUDE (CIDADE_END, COD_UF, PERC_END, DTINCLUSAO_END/*, ORDEM*/)                                                  
  CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO (CPF_DEV, COD_UF) INCLUDE(CIDADE_END, PERC_END, DTINCLUSAO_END/*, ORDEM*/)                                                  
 END                                                    
 ...
 IF OBJECT_ID ('SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO') IS NULL                                                    
 BEGIN                                        
  SELECT CONTRATO_FIN, CARTEIRA, CASE WHEN NOMEPRODUTO = '' THEN NULL ELSE NOMEPRODUTO END AS NOMEPRODUTO, 2 AS FLAG, A.COD_EMPRESA                                                  
   INTO SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO     
  FROM                              
   [SRC].dbo.AUX_BRADESCOBANCO AS A  (NOLOCK)                         
   LEFT JOIN [SRC].dbo.BRADESCOPRODUTOS AS B ON (B.CODPRODUTO = A.CARTEIRA)                                                       
  WHERE                                   
EXISTS (SELECT * FROM SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO AS C WHERE A.CONTRATO_FIN = C.CONTRATO_FIN)                                
                                                    
  SET @QTD_REGISTROS = @@ROWCOUNT                                                    
                                                    
  CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO (CONTRATO_FIN) INCLUDE(CARTEIRA, NOMEPRODUTO, FLAG)                                                    
 END                                       
...
------------------------------------------------------------------------------------------------------                                                  
 ...
 IF OBJECT_ID ('SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO') IS NULL                            
 BEGIN                                           
 SELECT                                          
  ...
  INTO SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO                                          
 FROM                                          
 ...
 WHERE                                          
  ...
  --CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO (CONTRATO_FIN) INCLUDE(CARTEIRA, NOMEPRODUTO, FLAG)                                                    
 END                                                    
 ...
 SELECT *                                              
  INTO SRC.DeepCenter.TBL_JUNCAO_ACIONA_DISCADOR                                                    
 FROM (                                                  
 ...
 LEFT(TRY_CONVERT(VARCHAR(50), TRY_CONVERT(TIME, A.DATA_ACIONA)), 8) AS HORA /*B.HORA*/, NULL AS IDCALL_SINERGYTECH, B.MAILING         
  FROM SRC.DeepCenter.ACIONA_BRADESCO_SELECAO AS A (NOLOCK)                                                    
  JOIN SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO AS C (NOLOCK) ON C.ID_ACIONA = A.ID_ACIONA                                                    
  JOIN SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2 AS B (NOLOCK) ON B.IDCALL = C.IDCALL /*AND B.CONTRATO = A.CONTRATO_FIN*/                                                  
  ...
  FROM SRC.DeepCenter.ACIONA_BRADESCO_SELECAO AS A (NOLOCK)                                                  
  LEFT JOIN SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO AS C (NOLOCK) ON C.ID_ACIONA = A.ID_ACIONA                                            
  ...
  SELECT                                                   
   ...
   LEFT(TRY_CONVERT(VARCHAR(50), TRY_CONVERT(TIME, A.DATA_ACIONA)), 8) AS HORA, NULL AS IDCALL_SINERGYTECH, NULL AS MAILING              
  FROM                                                   
 SRC.DeepCenter.ACIONA_BRADESCO_SELECAO AS A (NOLOCK)                                                  
 JOIN SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO AS C (NOLOCK) ON C.CONTRATO_FIN = A.CONTRATO_FIN                
 CROSS APPLY (SELECT TOP 1 RTRIM(LTRIM(B.DDD_TEL))+RTRIM(LTRIM(B.TEL_TEL)) AS PHONENUMBER                                                  
     FROM SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO AS B (NOLOCK)                                                    
     WHERE B.CPF_DEV = C.CPF_DEV AND LEN(B.TEL_TEL)>=9                                                  
     ORDER BY B.POSSUIWHATSAPP_TEL, B.PERC_TEL DESC, B.COD_TEL DESC) AS B                                                  
 LEFT JOIN SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO AS D (NOLOCK) ON D.ID_ACIONA = A.ID_ACIONA  AND TIPO_LIGACAO = 2 /*AND TIPO_LIGACAO = 2 ACRESCENTADO NA OS 156136*/                                   
  WHERE                                                   
 ...
 SRC.DeepCenter.ACIONA_BRADESCO_SELECAO AS A (NOLOCK)                                                  
 JOIN SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO AS C (NOLOCK) ON C.ID_ACIONA = A.ID_ACIONA                                                    
 JOIN SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO AS B (NOLOCK) ON C.IDCALL = B.IDCALL_SINERGYTECH                                               
 ...
 SET @ID_LOG = SCOPE_IDENTITY()                                                    
                    --DROP TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX                                
 IF OBJECT_ID ('SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX') IS NULL                                                    
 BEGIN                                                 
                            
  SELECT ROW_NUMBER() OVER(ORDER BY [DATA], HORA) AS ID,*                               
   INTO SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX                                                   
  FROM (           
   SELECT                                                     
 ...
 ,A.ID_ENQUETE, A.ID_RESPOSTA
   FROM                              
    SRC.DeepCenter.TBL_JUNCAO_ACIONA_DISCADOR AS A (NOLOCK)                                                    
    JOIN SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO AS E (NOLOCK) ON E.CONTRATO_FIN = A.CONTRATO_FIN                                                    
    LEFT JOIN SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO AS F (NOLOCK) ON F.CONTRATO_FIN = E.CONTRATO_FIN                                                    
    JOIN [SRC].DBO.CAD_RECUP AS G (NOLOCK) ON G.COD_RECUP = A.COD_RECUP                                                    
    LEFT JOIN SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO AS H (NOLOCK) ON H.CONTRATO_FIN = E.CONTRATO_FIN                                                        
                                                    
    OUTER APPLY (SELECT RTRIM(LTRIM(I.DDD_TEL))+RTRIM(LTRIM(I.TEL_TEL)) AS PHONENUMBER                                                  
     FROM SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO AS I (NOLOCK)                               
     WHERE  (I.CPF_DEV = E.CPF_DEV AND I.TEL_TEL = A.TEL_TEL) ) AS I                                                        
                                                    
    LEFT JOIN SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO AS L (NOLOCK) ON L.CONTRATO_FIN = E.CONTRATO_FIN                                                    
                                                      
 --OUTER APPLY( SELECT TOP 1 CIDADE_END FROM SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO AS P                                                     
 --    WHERE P.CPF_DEV = E.CPF_DEV ORDER BY P.ORDEM, P.DTINCLUSAO_END DESC, P.PERC_END DESC ) AS P                                                    
 OUTER APPLY( SELECT TOP 1 COD_UF, CIDADE_END FROM SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO AS P                                                     
 ...
 LEFT JOIN TBL_BRADESCO_PORTOLIO_35 AS R WITH (NOLOCK) ON R.CPF_DEV = E.CPF_DEV                                                   
   ...
   OUTER APPLY (SELECT TOP 1 * FROM SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO (NOLOCK) AS K WHERE K.CONTRATO_FIN = A.CONTRATO_FIN) AS K                                          
 ...
 CREATE CLUSTERED INDEX CL_IX ON SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX (IDCALL, IDOCORRENCIA)                                
 ...
 FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX A                                
 ...
 SELECT *                                  
 INTO SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC                                  
 FROM CTE                                  
 WHERE RW2 = 1                               
                               
 --select * from DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC                              
                                
 --DROP TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC                                
                                
 --SELECT * FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_AUX                                
                                  
 SET @QTD_REGISTROS = @@ROWCOUNT                        
                                                    
 --ALTER TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC ADD ID INT IDENTITY(1,1)                                                     
                                                    
 UPDATE TBL_LOG_CRMDAC SET QTD_REGISTROS = @QTD_REGISTROS, DATA_FIN = GETDATE() WHERE ID = @ID_LOG                                              
              
 CREATE CLUSTERED INDEX CL_IX ON SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC (ID) -- OS167056                                           
                                           
 CREATE NONCLUSTERED INDEX NO_IX_IDCALL ON SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC(IDCALL) -- OS168731                                          
                                              
 CREATE NONCLUSTERED INDEX NO_IX ON SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC(IDOCORRENCIA, AGENTE) -- OS167056                 
                 
 CREATE NONCLUSTERED INDEX NO_IX_CANALACIONAMENTO_IDCALL ON SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC(COD_CANALACIONAMENTO, TIPOATENDIMENTO)                
                 
 UPDATE A SET COD_CANALACIONAMENTO = 1                
 FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC A                
 ...
 JOIN DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC AS B ON A.ID_ENQUETE = B.ID_ENQUETE AND A.ID_RESPOSTA = B.ID_RESPOSTA        
 ...
 DELETE FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC WHERE IDOCORRENCIA = 2000016 AND AGENTE = 'OLOS' -- OS167056                                  
 ...
 --select * from SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC                                
 ...
 DSNOMEASSESSORIA,DTDATAINSERCAO,DTDATAREFERENCIA,HRHORAINICIO,HRHORAFIM,NUMTEMPOFALADO,NUMTEMPOESPERA,DSIDAGENT,DSNOMEAGENT,NUMTELEFONE                                
  ...
  SELECT                                                       
  'MLGomes',GETDATE(),DATA,HORAINICIO,HORAFIM,TEMPOFALANDO,TEMPOESPERA,LEFT(IDAGENT,50),LEFT(AGENTE,100),LEFT(PHONENUMBER,11),                                
   LEFT(CONTRATO,100),IDCUSTOMER,LEFT(IDCUSTOMER,14),3,1,TIPODISCAGEM,/*TIPOATENDIMENTO*/idOrigemAtendimento,COD_CANALACIONAMENTO/*COALESCE(COD_TIPODISCAGEMDEEPCENTER,COD_CANALACIONAMENTO,1)*/,NULL,LEFT(IDCALL,100),                                
   DESLIGADOPOR,COALESCE(CODACIONAMENTO_DEEP,COD_OCORRENCIA,IDOCORRENCIA),COALESCE(COD_OCORRENCIA,IDOCORRENCIA),COALESCE(IDMOTIVOINAD,0),VLRATRASO,PARCELAATRASO,LEFT(IDGRAVACAO,100),/*DESCOCORRENCIA*/NULL,0/*COALESCE(PARCELAATRASO,1) */                  



FROM                                                    
   SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC AS A                                                    
  --WHERE                                                                                           
  ...
  SELECT IDCALL, ID_ACIONA, IDCALL_SINERGYTECH FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC AS A    
