...
/****** Object:  StoredProcedure [dbo].[ExecucaoDeepCenterBradescoComercialCrmDac_new]    Script Date: 19/05/2025 14:19:52 ******/
SET ANSI_NULLS ON
GO
...
,LOJISTA     VARCHAR(50)                                                      
 ...
 IF OBJECT_ID ('SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX') IS NOT NULL                                                      
 BEGIN                                                      
  DROP TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX                                                      
 END                                     
 ...
 BEGIN                                      
  ...
  WHERE                                                       
	CAST(DATA_ACIONA AS DATE) between '20250512' AND '20250516'   and                                               
    EXISTS (SELECT * FROM [SRC].DBO.CAD_DEVF AS B (NOLOCK) WHERE A.CONTRATO_FIN = B.CONTRATO_FIN AND B.COD_CLI IN (3,11,16)             
 ...
 INTO SRC.DeepCenter.ACIONA_BRADESCO_SELECAO_REP                                      
 FROM CTE A                                      
 WHERE SEQNUM = 1                                      
 --AND NOT EXISTS (SELECT * FROM SRC.DeepCenter.TBL_CONTROLE_DEEPCENTER_CRMDAC AS C WHERE A.ID_ACIONA = C.ID_ACIONA)                                      
  ...
  CREATE NONCLUSTERED INDEX NON_IX10 ON SRC.DeepCenter.ACIONA_BRADESCO_SELECAO_REP ([TEL_TEL])                                                       
  INCLUDE ([ID_ACIONA],[CONTRATO_FIN],[DATA_ACIONA],[COD_RECUP],[COD_ACIONAMENTO])                                                      
 END    
 
 --SELECT * FROM DeepCenter.ACIONA_BRADESCO_SELECAO_REP
 ...
 FROM                                                       
  ...
  EXISTS (SELECT * FROM SRC.DeepCenter.ACIONA_BRADESCO_SELECAO_REP AS B WHERE A.ID_ACIONA = B.ID_ACIONA)                                                      
                                                    
 SELECT                                             
  * INTO SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO_REP                                                      
 FROM (                                                    
...
SELECT        
   ...
   INTO SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2_REP                             
   FROM                       
   SRC.[DEEPCENTER].DISCAGENS_OLOS AS A (NOLOCK)                                                      
  WHERE                                                      
   EXISTS (SELECT * FROM SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO_REP (NOLOCK) AS B WHERE A.IDCALL = B.IDCALL /*AND A.CONTRATO = B.CONTRATO_FIN*/)                                                  
  ...
  --CREATE NONCLUSTERED INDEX NON_IX1 ON SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2_REP (CONTRATO)                                                        
 END                                                      
 ...
 BEGIN                               SELECT                                                     
  ...
  INTO SRC.DeepCenter.TBL_RESULTADODISCAGEM_SINERGYTECH_BRADESCO_SELECAO_REP                                                  
 FROM TBL_RESULTADODISCAGEM_SINERGYTECH (NOLOCK) AS A                                                    
 WHERE EXISTS (SELECT * FROM SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO_REP (NOLOCK) AS B WHERE A.CALLRESULTID = B.IDCALL AND B.DISCADOR = 'SINERGYTECH')                                                    
 ...
 BEGIN                                                      
  SELECT                                                       
   ...
   ,B.EMAIL_DEV                                  
   ,A.PLANO_FIN                                  
   INTO SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO_REP                                            
  FROM                                                       
 ...
 BEGIN                                                    
  SELECT                                                       
   ...
   INTO SRC.DeepCenter.AUX_DEVF_BRADESCO_SELECAO_REP                                                      
  FROM                                                      
 ...
 BEGIN                                                      
  ...
  CONTRATO_FIN                   
   ...
   ,PARCELA_ACOP                                  
   ,TIPO_PARCACO                                                      
   INTO SRC.DeepCenter.CAD_ACOP_BRADESCO_SELECAO_REP                                                      
  FROM                                                       
   [SRC].DBO.CAD_ACOP AS A (NOLOCK)                                                      
  WHERE                                          
   EXISTS ( SELECT * FROM SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO_REP AS B WHERE A.CONTRATO_FIN = B.CONTRATO_FIN)                                                      
   AND COD_STPA = 0 AND CAST(VENC_ACOP AS DATE) < '20250512'                                   
 ...
 BEGIN                                          
  SELECT                                                       
   ...
   INTO SRC.DeepCenter.CAD_ACOP_AGREGACAO_BRADESCO_SELECAO_REP                                                      
  FROM                                                       
 ...
 SELECT CPF_DEV, DDD_TEL, TEL_TEL, PERC_TEL, POSSUIWHATSAPP_TEL, COD_TEL                                                    
   INTO SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO_REP                                            
  FROM                                                       
 ...
 BEGIN                                                      
  SELECT CPF_DEV, CIDADE_END, COD_UF, PERC_END, DTINCLUSAO_END, COD_END --CASE WHEN COD_TIPO > 2 THEN 9 ELSE COD_TIPO END AS ORDEM                                                  
   INTO SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO_REP                                                      
  FROM                                                       
 ...
 BEGIN                                          
  SELECT CONTRATO_FIN, CARTEIRA, CASE WHEN NOMEPRODUTO = '' THEN NULL ELSE NOMEPRODUTO END AS NOMEPRODUTO, 2 AS FLAG, A.COD_EMPRESA                                                    
   INTO SRC.DeepCenter.AUX_BRADESCOBANCO_BRADESCO_SELECAO_REP       
  FROM                                
...
------------------------------------------------------------------------------------------------------                                                    
 ...
 SELECT                                            
  ...
  INTO SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO_REP                                            
 FROM                                            
 ...
 WHERE                                            
  ...
  --CREATE NONCLUSTERED INDEX NON_IX ON SRC.DeepCenter.AUX_BRADESCOLPTITULO_BRADESCO_SELECAO_REP (CONTRATO_FIN) INCLUDE(CARTEIRA, NOMEPRODUTO, FLAG)                                                      
 END   
 ...
 SELECT *                                                
  INTO SRC.DeepCenter.TBL_JUNCAO_ACIONA_DISCADOR_REP                                                      
 FROM (                                                    
 ...
 -- FROM SRC.DeepCenter.ACIONA_BRADESCO_SELECAO_REP AS A (NOLOCK)                                                      
 -- JOIN SRC.DeepCenter.TAB_IDLIGACAODISCADOR_ACIONA_BRADESCO_SELECAO_REP AS C (NOLOCK) ON C.ID_ACIONA = A.ID_ACIONA                                                      
 -- JOIN SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2_REP AS B (NOLOCK) ON B.IDCALL = C.IDCALL /*AND B.CONTRATO = A.CONTRATO_FIN*/                                                    
 -- --AND CAST(A.DATA_ACIONA AS DATE) = B.[DATA] AND B.PHONENUMBER = RTRIM(LTRIM(A.DDD_TEL)) + RTRIM(LTRIM(A.TEL_TEL))                                                    
 -- WHERE A.TIPOCANALDEEPCENT_ACIONAMENTO NOT IN (4, 5, 10) -- ADICIONADO O 5 NA OS OS 171740                                             
 ...
 -- UNION ALL                                                    
  ...
  WHERE A.TIPOCANALDEEPCENT_ACIONAMENTO IN (5)  -- ADICIONADO O 5 NA OS OS 171740                                                  
                                                    
 -- UNION ALL                                                     
 -- --ESSE SELECT ABAIXO NÃO FAZ SENTIDO, POIS AO CRIAR A SRC.DeepCenter.ACIONA_BRADESCO_SELECAO_REP, JÁ ESTAMOS EXCLUINDO TODO MUNDO QUE   
 -- --O TIPOCANALDEEPCENT_ACIONAMENTO É DIFERENTE DE 10, ENTÃO ESSE SELECT NUNCA VAI RETORNAR NADA                                                    
 -- SELECT                                                     
 ...
 --    FROM SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO_REP AS B (NOLOCK)                                                      
 ...
 --SET @ID_LOG = SCOPE_IDENTITY()                                                      
                    --DROP TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX                                  
 IF OBJECT_ID ('SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX') IS NULL                                                      
 BEGIN                                                   
                              
  SELECT ROW_NUMBER() OVER(ORDER BY [DATA], HORA) AS ID,*                                 
   INTO SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX                                                     
  FROM (             
   SELECT                                                       
 ...
 ,A.ID_ENQUETE, A.ID_RESPOSTA  
   FROM                                
    ...
    JOIN SRC.DeepCenter.CAD_DEVF_BRADESCO_SELECAO_REP AS E (NOLOCK) ON E.CONTRATO_FIN = A.CONTRATO_FIN                                                      
    ...
    OUTER APPLY (SELECT RTRIM(LTRIM(I.DDD_TEL))+RTRIM(LTRIM(I.TEL_TEL)) AS PHONENUMBER                                                    
     FROM SRC.DeepCenter.CAD_DEVT_BRADESCO_SELECAO_REP AS I (NOLOCK)                                 
 ...
 --OUTER APPLY( SELECT TOP 1 CIDADE_END FROM SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO_REP AS P                                                       
 --    WHERE P.CPF_DEV = E.CPF_DEV ORDER BY P.ORDEM, P.DTINCLUSAO_END DESC, P.PERC_END DESC ) AS P                                                      
 OUTER APPLY( SELECT TOP 1 COD_UF, CIDADE_END FROM SRC.DeepCenter.CAD_DEVE_BRADESCO_SELECAO_REP AS P                                                       
 ...
 CREATE CLUSTERED INDEX CL_IX ON SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX (IDCALL, IDOCORRENCIA) 
 

  SELECT IDOCORRENCIA, TIPODISCAGEM FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX 
  SELECT COD_ACIONAMENTO, COD_RESULTADO AS CODACIONAMENTO_DEEP, COD_OCORRENCIA  FROM DEEPCENTER.CONFIG_ACIONAMENTO ORDER BY COD_ACIONAMENTO
  SELECT * FROM DEEPCENTER.CONFIG_TIPODISCAGEM
 ...
 ,coalesce(C.COD_TIPODISCAGEMDEEPCENTER, A.TIPODISCAGEM) as IDTIPODISCAGEM   
 ...
 --,COALESCE(COD_CANALACIONAMENTO,1) as COD_CANALACIONAMENTO  
  
 FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX A                                  
 ...
 SELECT *,  
 CASE  -- OS 171742    
  WHEN COD_CANALACIONAMENTO IN (1,9) THEN 0  
  WHEN COD_CANALACIONAMENTO = 17 AND TIPOATENDIMENTO = 0 THEN 0  
  ELSE 1  
 END AS idOrigemAtendimento   
 INTO SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP                                    
 FROM CTE                                    
 WHERE RW2 = 1   
 ...
 --SELECT * FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP_AUX                                  
                                    
 --SET @QTD_REGISTROS = @@ROWCOUNT                          
                                                      
 --ALTER TABLE SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP ADD ID INT IDENTITY(1,1)                                                       
 ...
 FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP A                  
 ...
 JOIN DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP AS B ON A.ID_ENQUETE = B.ID_ENQUETE AND A.ID_RESPOSTA = B.ID_RESPOSTA          
 ...
 SET @VLR_FIN = @BLOCO                          
  ...
  LEFT(CONTRATO,100), IDCUSTOMER, LEFT(IDCUSTOMER,14), 3, 1, IDTIPODISCAGEM, idOrigemAtendimento, COD_CANALACIONAMENTO, NULL, LEFT(IDCALL,100),                                  
  ...
  SELECT IDCALL, ID_ACIONA, IDCALL_SINERGYTECH FROM SRC.DeepCenter.TBL_JUNCAO_BRADESCO_CRMDAC_REP AS A      
 ...
 --IF OBJECT_ID ('SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2_REP') IS NOT NULL                                   --BEGIN                                                      
 -- DROP TABLE SRC.DeepCenter.TBL_DEEPCENTER_DISCADOR_OLOS_BRADESCO_SELECAO_2_REP                                                      
 --END                          
