CREATE PROCEDURE SHIRINK_TESTE
AS
BEGIN
	-- #############################################
	...
	DECLARE @FileName sysname = N'lx_zero_300_dbData';
	...
	SELECT @CurrentSizeMB = cast(size as bigint) * 8 / 1024
	...
	IF DATEPART(HOUR, GETDATE()) < 19 AND DATEPART(HOUR, GETDATE()) >= 7  
	BEGIN   
	...
	WHILE (@CurrentSizeMB > @TargetSizeMB) OR (DATEPART(HOUR, GETDATE()) >= 19 OR DATEPART(HOUR, GETDATE()) < 7)
	BEGIN
