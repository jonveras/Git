...
WHERE 
    ...
    AND (sp.modification_counter > 1000 -- Mudanças relevantes
    OR (CAST(sp.rows_sampled AS FLOAT) / NULLIF(sp.rows, 0)) * 100 < 90 -- Amostragem ruim
    OR CAST(sp.last_updated AS DATE) <= GETDATE() - 3) -- Atualização antiga
GROUP BY 
    s.name, o.name
ORDER BY 
...
--	Query que mostra algumas sugestões de índices para que possamos analisar a criação
SELECT 
